...
final class FinRunner: @unchecked Sendable {
    ...
    func start(prompt: String, continuation: Continuation, model: String?) {
        ...
        // Tag every session started from beacon so the picker only shows our own.
        // For .fresh this tags the new session; for .last it filters to our last one.
        if case .session = continuation { } else { args += ["-tag", "beacon"] }
    ...
    func listSessions(limit: Int,
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            for (url, mtime) in recent {
                ...
                guard h.tags?.contains("beacon") == true else { continue }
